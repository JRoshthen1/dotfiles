...
awful.spawn.with_shell("sleep 1 && picom --config ~/.config/picom/picom.conf")
awful.spawn.with_shell("sleep 1 && xautolock -time 5 -locker /home/jrosh/.local/bin/lock")
awful.spawn.with_shell("sleep 1 && /usr/bin/libinput-gestures-setup restart")
awful.spawn.with_shell("sleep 1 && clipse -listen")
...
globalkeys = mytable.join(
	...
	awful.key({ altkey }, "p", function()
		...
		local filename = os.getenv("HOME") .. "/Pictures/screenshots/" .. timestamp .. ".png"
		os.execute("mkdir -p " .. os.getenv("HOME") .. "/Pictures/screenshots")
		awful.spawn.with_shell("scrot '" .. filename .. "'")

		-- Wait a moment for the file to be written, then show notification
		gears.timer.start_new(0.2, function()
			naughty.notify({
				title = "Screenshot Captured",
				text = "Saved as: " .. timestamp .. ".png",
				timeout = 3,
				urgency = "normal",
				icon = filename,
				icon_size = 64,
	...
	awful.key({ modkey }, "F1", function()
		os.execute(string.format("xsel --primary --output | %s", "~/.local/bin/ttsui"))
	end, { description = "Speak text", group = "launcher" }),
  --]]
	--[[ rofi--]]
	awful.key({ modkey }, "space", function()
		os.execute(string.format("rofi -show %s -theme %s", "drun", "~/.config/rofi/themes/theme.rasi"))
	...
	awful.key({ modkey }, "v", function()
		awful.spawn(string.format("%s --class=clipse -e clipse", terminal))
	end, { description = "toggle Clipse clipboard manager", group = "Clipboard" }),
...
awful.rules.rules = {
	...
	{
		rule_any = { class = { ".*scratchpad.*", ".*clipse.*" } },
		--rule_any = { class = { "scratchpad", "terminal-scratchpad", "clipse-scratchpad" } },
		properties = {
			floating = true,
			sticky = true,
			ontop = true,
